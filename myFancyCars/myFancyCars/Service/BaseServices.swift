...
class BaseServices {
  ...
  private func parseData(data: Data?) -> ResponseData? {
    ...
    if json.type != .null {
      print(" 📡 \(json) 📡")
      return ResponseData(type: .json, json: json, xml: nil, string: nil)
    } else if (xml.element?.children.count ?? 0 > 0 && !isHTML) {
        print(" 📡 \(xml) 📡")
      return ResponseData(type: .xml, json: nil, xml: xml, string: nil)
    } else {
      if let respStr = String(data: data!, encoding: .utf8) {
        print(" 📡" + respStr + "📡")
        return ResponseData(type: .string, json: nil, xml: nil, string: respStr)
      } else {
